...
function PromotionGateBanner({ gateStatus, summary }) {
...
function CandidateRow({ candidate, client, runId, onActionComplete }) {
  ...
  return (
    <div className={`ceu-candidate-row ceu-candidate-row--${candidate.state.toLowerCase()}`}>
      <div className="ceu-candidate-row__header" onClick={() => setExpanded(!expanded)}>
        <span className="ceu-candidate-row__expand">{expanded ? '▾' : '▸'}</span>
...
function ObligationItem({ obligation, client, runId, onActionComplete }) {
  const [actorId, setActorId] = useState('operator:khorrix');
  const [justification, setJustification] = useState('');
  const [pending, setPending] = useState(false);
  const [error, setError] = useState(null);
  ...
  return (
    <div className={`ceu-obligation-item ceu-obligation-item--${obligation.status.toLowerCase()}`}>
      <div className="ceu-obligation-item__header">
        ...
        <span className="ceu-obligation-item__ceu">{obligation.ceu_id}</span>
...
export default function CeuReconciliationPanel({ ceuData }) {
  ...
  return (
    <div className="ceu-reconciliation-panel" key={refreshKey}>
      <PromotionGateBanner gateStatus={ceuData.gateStatus} summary={ceuData.summary} />
        ...
        {ceuData.obligationQueue && ceuData.obligationQueue.total > 0 && (
          <div className="ceu-section">
            ...
            <div className="ceu-obligation-list">
              {ceuData.obligationQueue.items.map(obl => (
                <ObligationItem
