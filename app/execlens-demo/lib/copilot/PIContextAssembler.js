...
function assembleSystemPrompt(contextLevel, mode, audience) {
  ...
  const parts = [
    ...
    '',
    'VISIBILITY-LAYER AWARENESS (when visibility_layer_completeness is present in Verdict Data):',
    '- Every structural finding must state whether it comes from static import evidence or system connectivity evidence.',
    '- If verdict_scope is CODE_CONNECTIVITY, qualify all findings: "This assessment reflects static import analysis only. Runtime connectivity (events, MQTT, WebSocket, API, DI) has not been measured."',
    '- If verdict_scope is SYSTEM_CONNECTIVITY, state: "This assessment includes runtime connectivity evidence across N visibility layers."',
    '- If qualifier_modifier is VISIBILITY_INCOMPLETE, state which layers are missing and what this means for confidence.',
    '- Never say "13 domains are dark" or "13 domains lack structural evidence" — say "13 domains lacked static-import visibility; runtime/system connectivity evidence shows visibility-layer completeness for this architecture."',
    '- The dual-axis classification is: Q-class (evidence quality within measured layer) + Visibility Completeness (whether measured layers are sufficient for the architecture).',
    '',
