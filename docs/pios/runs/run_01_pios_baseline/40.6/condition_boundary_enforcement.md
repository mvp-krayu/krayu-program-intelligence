...
|---|---|---|
| Layer 1 — Telemetry (40.4) | Raw measurement extraction | Read-only via 40.5 signal inputs; not directly accessed |
...
|---|---|---|
| 40.5 signal_output_set.md | Read | Authorized — primary input |
| 40.5 signal_validation_report.md | Read | Authorized — coverage state reference |
...
| Condition validation report | Produced — authorized output |
...
| No threshold definition | Compliant — thresholds are Stream 75.1 authority; not defined in this stream |
| No condition without CKR trace | Compliant — all 8 conditions trace to CKR-012 |
...
|---|---|
| GC-07 — State–Diagnosis Separation | Conditions declare activation readiness (evaluable / partial / blocked); no diagnosis output produced |
| GC-06 — Evidence-First | Blocked signals propagate as blocked conditions; no activation fabricated for unavailable signals |
...
|---|---|---|
| Stream 40.7 (if defined) — Diagnosis | docs/pios/40.6/ (full corpus) | 8 governed conditions with activation states, signal input values, traceability, and coverage state declarations |
| Stream 75.2 — Program Diagnosis Model | docs/pios/40.6/condition_output_set.md | Evaluable conditions (COND-001, COND-002) with full signal input values; partial conditions (COND-003, COND-004, COND-007, COND-008) with available components; blocked conditions (COND-005, COND-006) with blocking reasons |
