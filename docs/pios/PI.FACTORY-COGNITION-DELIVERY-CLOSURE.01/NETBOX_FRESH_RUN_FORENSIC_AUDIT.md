...
**[CORRECTED 2026-06-12 — see EVOLUTION_RECONSTRUCTION.md]** The earlier claim here of "two distinct pipeline lineages / different code paths" was an inference from artifact presence and is **withdrawn**. Git history shows **one** orchestrator (`run_client_pipeline.py`, since 2026-05-01) whose governed chain is *partially integrated*: semantic phases `03b/03c` exist but are **opt-in** (`--enable-semantic-derivation` default OFF); vault `08a/08b` exist but are **conditional** on cognition inputs; SQO and observation/convergence are **not orchestrator phases at all** (separate `promotion_action.py` / `genesis_compiler.py` subsystems). Historical specimen runs look complete because those pieces were supplied via enabled flags + separate scripts during May development. The fresh run with defaults exposes the incomplete integration. The artifact-presence differences across runs reflect *invocation completeness*, not different pipelines.
