...
| No invented artifacts | CONFIRMED |

---

## Files Modified

`scripts/pios/source_intake.py` only:
- `REQUIRED_MANIFEST_FIELDS` — `extracted_path` removed
- `resolve_inventory_source_path()` — added
- `step_inventory()` — updated signature and hybrid path logic
- `build_intake_manifest()` — `.get("extracted_path", "")` fix
- `main()` — `step_inventory(manifest, client_id, run_id)` call updated
...
`--dry-run` — no output files written, no directories created.
Run ID `run_blueedge_intake_contract_closure_01` used. No files written.
