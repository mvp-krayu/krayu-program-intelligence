...
The canonical score is the proven floor. coverage_points=35 is derived as round(100.0 × 0.35). reconstruction_points=25 is a categorical award for a PASS verdict. completion_points=0 because the execution layer has not been evaluated (execution_layer_evaluated=False). The execution layer can award up to 40 additional points when run. Computed in S4 by the pios compute gauge stage.
