...
const TABS = [
  ...
  { id: 'regulations', label: '🏛️ Regulations' },
...
export default function CrossBorderPage() {
  ...
  const kpis = [
    { title: 'Active Corridors', value: 14, trend: 2, sparkline: spark(14) },
    { title: 'GCC Countries', value: 6, trend: 0, sparkline: spark(6) },
    { title: 'Compliance Rate', value: '97.2%', trend: 1.8, sparkline: spark(50) },
    { title: 'Crossings (30d)', value: 342, trend: 8, sparkline: spark(342) },
    { title: 'Violations', value: 3, trend: -2, sparkline: spark(3) },
    { title: 'Avg Crossing', value: '28 min', trend: -4, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'CRO'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Cross-Border Compliance" breadcrumb="Cross-Border" subtitle="GCC & international border regulations for fleet transit" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'matrix' && (<>
        <div className="grid grid-2">
          <ChartCard title="Cross-Border Compliance — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [75, 41, 55, 52, 81, 77], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [70, 34, 49, 31, 40, 53], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [11, 17, 33, 29, 18], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={73.3} max={100} thresholds={[60, 80]} trend={1.4} />
          <GaugeChart title="Efficiency" value={84.1} max={100} thresholds={[55, 75]} trend={4.4} />
          <GaugeChart title="Compliance" value={75.9} max={100} thresholds={[80, 90]} trend={2.7} />
          <GaugeChart title="Utilization" value={76.1} max={100} thresholds={[50, 75]} trend={2.3} />
        </div>
      </>)}

      {tab === 'regulations' && (
        <TableCard title="Regulations" count={tableRows.length}>
      ...
      {tab === 'routes' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [57, 70, 25, 69, 63, 44], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [43, 83, 52, 40, 40, 77], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [36, 43, 41, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={71.3} max={100} thresholds={[70, 85]} trend={2.9} />
          <GaugeChart title="Coverage" value={73.0} max={100} thresholds={[60, 80]} trend={3.7} />
          <GaugeChart title="Accuracy" value={93.1} max={100} thresholds={[80, 90]} trend={3.4} />
          <GaugeChart title="Speed" value={84.4} max={100} thresholds={[50, 75]} trend={0.6} />
        </div>
      </>)}

      {tab === 'analytics' && (
        <TableCard title="Analytics" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'permits' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [57, 63, 66, 58, 64, 59], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [36, 43, 41, 34], backgroundColor: '#10b981' },
            { label: 'Pending', data: [18, 9, 11, 15], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [5, 7, 5, 6], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={83.3} max={100} thresholds={[65, 80]} trend={-0.3} />
          <GaugeChart title="Trend" value={70.1} max={100} thresholds={[55, 75]} trend={1.1} />
          <GaugeChart title="Target Hit" value={86.9} max={100} thresholds={[70, 85]} trend={0.8} />
          <GaugeChart title="Velocity" value={69.9} max={100} thresholds={[50, 70]} trend={0.6} />
        </div>
      </>)}
