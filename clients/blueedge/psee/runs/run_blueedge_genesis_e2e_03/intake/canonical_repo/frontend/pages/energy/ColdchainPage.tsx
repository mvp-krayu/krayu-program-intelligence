...
export default function ColdchainPage() {
  ...
  const kpis = [
    { title: 'Active Shipments', value: 28, trend: 3, sparkline: spark(28) },
    { title: 'Normal', value: 24, trend: 2, sparkline: spark(24) },
    { title: 'Warning', value: 3, trend: 1, sparkline: spark(3) },
    { title: 'Breached', value: 1, trend: -1, sparkline: spark(1) },
    { title: 'Avg Temp', value: '-18.2°C', trend: 0.3, sparkline: spark(50) },
    { title: 'Sensor Alerts', value: 5, trend: -2, sparkline: spark(5) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'COL'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Cold Chain Monitoring" breadcrumb="Cold Chain" subtitle="Temperature-controlled cargo monitoring and breach detection" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'monitor' && (<>
        <div className="grid grid-2">
          <ChartCard title="Cold Chain Monitoring — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [45, 57, 95, 80, 69, 82], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [67, 33, 74, 59, 50, 61], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [22, 41, 37, 32, 16], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={66.4} max={100} thresholds={[60, 80]} trend={3.5} />
          <GaugeChart title="Efficiency" value={84.2} max={100} thresholds={[55, 75]} trend={2.9} />
          <GaugeChart title="Compliance" value={82.3} max={100} thresholds={[80, 90]} trend={1.0} />
          <GaugeChart title="Utilization" value={84.5} max={100} thresholds={[50, 75]} trend={-0.1} />
        </div>
      </>)}

      {tab === 'shipments' && (
        <TableCard title="Shipments" count={tableRows.length}>
      ...
      {tab === 'sensors' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [78, 49, 75, 28, 32, 33], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [43, 51, 70, 52, 82, 80], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [45, 24, 37, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={88.0} max={100} thresholds={[70, 85]} trend={-0.2} />
          <GaugeChart title="Coverage" value={77.5} max={100} thresholds={[60, 80]} trend={1.7} />
          <GaugeChart title="Accuracy" value={84.2} max={100} thresholds={[80, 90]} trend={1.9} />
          <GaugeChart title="Speed" value={79.9} max={100} thresholds={[50, 75]} trend={0.5} />
        </div>
      </>)}

      {tab === 'breaches' && (
        <TableCard title="Breaches" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [82, 77, 86, 76, 86, 56], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [45, 24, 37, 34], backgroundColor: '#10b981' },
            { label: 'Pending', data: [5, 8, 11, 11], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [2, 8, 2, 6], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={83.8} max={100} thresholds={[65, 80]} trend={2.5} />
          <GaugeChart title="Trend" value={73.7} max={100} thresholds={[55, 75]} trend={1.1} />
          <GaugeChart title="Target Hit" value={93.7} max={100} thresholds={[70, 85]} trend={3.1} />
          <GaugeChart title="Velocity" value={79.5} max={100} thresholds={[50, 70]} trend={3.2} />
        </div>
      </>)}
