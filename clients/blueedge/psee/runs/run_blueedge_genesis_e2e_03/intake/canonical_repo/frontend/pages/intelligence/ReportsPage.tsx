...
export default function ReportsPage() {
  ...
  const kpis = [
    { title: 'Total Reports', value: 856, trend: 42, sparkline: spark(856) },
    { title: 'Scheduled', value: 24, trend: 3, sparkline: spark(24) },
    { title: 'Generated Today', value: 12, trend: 2, sparkline: spark(12) },
    { title: 'Avg Generation', value: '4.2s', trend: -0.5, sparkline: spark(50) },
    { title: 'Templates', value: 38, trend: 4, sparkline: spark(38) },
    { title: 'Subscribers', value: 67, trend: 8, sparkline: spark(67) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'REP'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Reports Center" breadcrumb="Reports" subtitle="Generate, schedule, and manage fleet reports" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'templates' && (<>
        <div className="grid grid-2">
          <ChartCard title="Reports Center — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [78, 78, 92, 68, 67, 44], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [32, 49, 45, 41, 34, 73], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [39, 30, 43, 45, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={77.4} max={100} thresholds={[60, 80]} trend={4.4} />
          <GaugeChart title="Efficiency" value={70.6} max={100} thresholds={[55, 75]} trend={2.5} />
          <GaugeChart title="Compliance" value={75.9} max={100} thresholds={[80, 90]} trend={0.7} />
          <GaugeChart title="Utilization" value={87.7} max={100} thresholds={[50, 75]} trend={0.3} />
        </div>
      </>)}

      {tab === 'scheduled' && (
        <TableCard title="Scheduled" count={tableRows.length}>
      ...
      {tab === 'generate' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [40, 27, 67, 51, 61, 75], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [44, 64, 67, 60, 40, 52], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [27, 38, 45, 27], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={83.8} max={100} thresholds={[70, 85]} trend={2.3} />
          <GaugeChart title="Coverage" value={87.8} max={100} thresholds={[60, 80]} trend={2.3} />
          <GaugeChart title="Accuracy" value={71.1} max={100} thresholds={[80, 90]} trend={-0.0} />
          <GaugeChart title="Speed" value={81.5} max={100} thresholds={[50, 75]} trend={4.5} />
        </div>
      </>)}

      {tab === 'history' && (
        <TableCard title="History" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [91, 70, 74, 92, 79, 75], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [27, 38, 45, 27], backgroundColor: '#10b981' },
            { label: 'Pending', data: [13, 7, 17, 15], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [5, 4, 3, 3], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={92.8} max={100} thresholds={[65, 80]} trend={4.1} />
          <GaugeChart title="Trend" value={78.1} max={100} thresholds={[55, 75]} trend={1.8} />
          <GaugeChart title="Target Hit" value={85.9} max={100} thresholds={[70, 85]} trend={0.1} />
          <GaugeChart title="Velocity" value={68.6} max={100} thresholds={[50, 70]} trend={3.7} />
        </div>
      </>)}
