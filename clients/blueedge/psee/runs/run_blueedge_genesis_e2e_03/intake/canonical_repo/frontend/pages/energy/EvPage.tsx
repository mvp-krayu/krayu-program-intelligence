...
export default function EvPage() {
  ...
  const kpis = [
    { title: 'EV Fleet', value: 45, trend: 5, sparkline: spark(45) },
    { title: 'Charging Now', value: 12, trend: 2, sparkline: spark(12) },
    { title: 'Avg SOC', value: '68%', trend: 3, sparkline: spark(50) },
    { title: 'Health', value: '94.2%', trend: 0.5, sparkline: spark(50) },
    { title: 'Range', value: '285 km', trend: 8, sparkline: spark(50) },
    { title: 'Energy', value: '1,240 kWh', trend: 95, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'EV'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="EV Fleet Management" breadcrumb="EV" subtitle="Electric vehicle battery health, charging, and range management" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'fleet' && (<>
        <div className="grid grid-2">
          <ChartCard title="EV Fleet Management — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [49, 94, 81, 56, 59, 86], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [54, 62, 43, 67, 47, 60], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [44, 30, 38, 35, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={84.9} max={100} thresholds={[60, 80]} trend={0.5} />
          <GaugeChart title="Efficiency" value={80.8} max={100} thresholds={[55, 75]} trend={0.7} />
          <GaugeChart title="Compliance" value={74.4} max={100} thresholds={[80, 90]} trend={2.3} />
          <GaugeChart title="Utilization" value={70.2} max={100} thresholds={[50, 75]} trend={0.6} />
        </div>
      </>)}

      {tab === 'charging' && (
        <TableCard title="Charging" count={tableRows.length}>
      ...
      {tab === 'battery' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [26, 51, 78, 68, 68, 57], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [71, 64, 44, 66, 66, 77], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [20, 39, 16, 48], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={92.4} max={100} thresholds={[70, 85]} trend={4.4} />
          <GaugeChart title="Coverage" value={75.3} max={100} thresholds={[60, 80]} trend={1.0} />
          <GaugeChart title="Accuracy" value={75.6} max={100} thresholds={[80, 90]} trend={0.9} />
          <GaugeChart title="Speed" value={88.2} max={100} thresholds={[50, 75]} trend={0.5} />
        </div>
      </>)}

      {tab === 'range' && (
        <TableCard title="Range" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [87, 82, 57, 60, 63, 67], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [20, 39, 16, 48], backgroundColor: '#10b981' },
            { label: 'Pending', data: [5, 3, 8, 5], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [6, 4, 6, 7], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={86.6} max={100} thresholds={[65, 80]} trend={3.0} />
          <GaugeChart title="Trend" value={84.3} max={100} thresholds={[55, 75]} trend={1.1} />
          <GaugeChart title="Target Hit" value={85.8} max={100} thresholds={[70, 85]} trend={1.2} />
          <GaugeChart title="Velocity" value={83.3} max={100} thresholds={[50, 70]} trend={4.2} />
        </div>
      </>)}
