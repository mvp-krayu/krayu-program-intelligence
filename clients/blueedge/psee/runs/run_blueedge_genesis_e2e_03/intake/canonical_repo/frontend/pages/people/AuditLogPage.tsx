...
export default function AuditLogPage() {
  ...
  const kpis = [
    { title: 'Events Today', value: 1245, trend: 85, sparkline: spark(1245) },
    { title: 'Users Active', value: 18, trend: 3, sparkline: spark(18) },
    { title: 'Creates', value: 342, trend: 12, sparkline: spark(342) },
    { title: 'Updates', value: 567, trend: 28, sparkline: spark(567) },
    { title: 'Deletes', value: 23, trend: -5, sparkline: spark(23) },
    { title: 'Failed', value: 8, trend: -2, sparkline: spark(8) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'AUD'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Audit Log" breadcrumb="Audit" subtitle="System-wide audit trail and compliance logging" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'log' && (<>
        <div className="grid grid-2">
          <ChartCard title="Audit Log — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [49, 66, 47, 74, 41, 62], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [55, 31, 59, 36, 51, 32], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [23, 17, 21, 37, 23], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={74.4} max={100} thresholds={[60, 80]} trend={0.1} />
          <GaugeChart title="Efficiency" value={85.3} max={100} thresholds={[55, 75]} trend={4.4} />
          <GaugeChart title="Compliance" value={93.7} max={100} thresholds={[80, 90]} trend={3.4} />
          <GaugeChart title="Utilization" value={76.9} max={100} thresholds={[50, 75]} trend={1.2} />
        </div>
      </>)}

      {tab === 'users' && (
        <TableCard title="By User" count={tableRows.length}>
      ...
      {tab === 'resources' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [66, 71, 39, 30, 26, 55], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [85, 52, 68, 74, 68, 73], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [48, 22, 16, 20], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={86.5} max={100} thresholds={[70, 85]} trend={1.6} />
          <GaugeChart title="Coverage" value={67.3} max={100} thresholds={[60, 80]} trend={1.4} />
          <GaugeChart title="Accuracy" value={85.7} max={100} thresholds={[80, 90]} trend={2.0} />
          <GaugeChart title="Speed" value={83.8} max={100} thresholds={[50, 75]} trend={1.2} />
        </div>
      </>)}

      {tab === 'security' && (
        <TableCard title="Security" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [83, 59, 85, 91, 59, 56], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [48, 22, 16, 20], backgroundColor: '#10b981' },
            { label: 'Pending', data: [5, 13, 11, 6], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [6, 6, 2, 5], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={68.1} max={100} thresholds={[65, 80]} trend={3.7} />
          <GaugeChart title="Trend" value={88.2} max={100} thresholds={[55, 75]} trend={3.6} />
          <GaugeChart title="Target Hit" value={90.5} max={100} thresholds={[70, 85]} trend={0.0} />
          <GaugeChart title="Velocity" value={83.0} max={100} thresholds={[50, 70]} trend={4.3} />
        </div>
      </>)}
