...
export default function CustomerPortalPage() {
  ...
  const kpis = [
    { title: 'Active Customers', value: 142, trend: 8, sparkline: spark(142) },
    { title: 'Shipments Today', value: 38, trend: 5, sparkline: spark(38) },
    { title: 'On-Time Rate', value: '96.8%', trend: 1.2, sparkline: spark(50) },
    { title: 'Avg Delivery', value: '4.2 hrs', trend: -0.3, sparkline: spark(50) },
    { title: 'SLA Breaches', value: 2, trend: -1, sparkline: spark(2) },
    { title: 'CSAT', value: '4.7/5', trend: 0.2, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'CUS'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Customer Portal" breadcrumb="Customer Portal" subtitle="Customer-facing shipment tracking and SLA management" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'dashboard' && (<>
        <div className="grid grid-2">
          <ChartCard title="Customer Portal — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [67, 90, 87, 45, 63, 55], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [30, 65, 59, 42, 63, 55], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [25, 10, 13, 41, 42], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={73.0} max={100} thresholds={[60, 80]} trend={0.5} />
          <GaugeChart title="Efficiency" value={88.6} max={100} thresholds={[55, 75]} trend={1.1} />
          <GaugeChart title="Compliance" value={68.2} max={100} thresholds={[80, 90]} trend={3.8} />
          <GaugeChart title="Utilization" value={91.2} max={100} thresholds={[50, 75]} trend={4.3} />
        </div>
      </>)}

      {tab === 'track' && (
        <TableCard title="Track" count={tableRows.length}>
      ...
      {tab === 'shipments' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [55, 71, 57, 33, 32, 42], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [80, 75, 78, 80, 46, 47], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [19, 42, 46, 29], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={90.8} max={100} thresholds={[70, 85]} trend={0.9} />
          <GaugeChart title="Coverage" value={71.7} max={100} thresholds={[60, 80]} trend={2.7} />
          <GaugeChart title="Accuracy" value={89.5} max={100} thresholds={[80, 90]} trend={1.5} />
          <GaugeChart title="Speed" value={78.8} max={100} thresholds={[50, 75]} trend={4.4} />
        </div>
      </>)}

      {tab === 'sla' && (
        <TableCard title="SLA Metrics" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'feedback' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [82, 67, 89, 85, 87, 85], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [19, 42, 46, 29], backgroundColor: '#10b981' },
            { label: 'Pending', data: [12, 5, 17, 16], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [8, 7, 7, 1], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={74.2} max={100} thresholds={[65, 80]} trend={2.2} />
          <GaugeChart title="Trend" value={88.9} max={100} thresholds={[55, 75]} trend={4.2} />
          <GaugeChart title="Target Hit" value={71.8} max={100} thresholds={[70, 85]} trend={0.1} />
          <GaugeChart title="Velocity" value={65.7} max={100} thresholds={[50, 70]} trend={4.4} />
        </div>
      </>)}
