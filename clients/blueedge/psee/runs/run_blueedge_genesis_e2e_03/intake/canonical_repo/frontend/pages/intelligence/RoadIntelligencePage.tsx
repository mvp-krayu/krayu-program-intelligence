...
export default function RoadIntelligencePage() {
  ...
  const kpis = [
    { title: 'Segments', value: 2840, trend: 120, sparkline: spark(2840) },
    { title: 'Hazards', value: 23, trend: -4, sparkline: spark(23) },
    { title: 'Quality Score', value: 7.8, trend: 0.3, sparkline: spark(7) },
    { title: 'Survey Vehicles', value: 45, trend: 5, sparkline: spark(45) },
    { title: 'Data Points', value: '12.4M', trend: 1.2, sparkline: spark(50) },
    { title: 'Last Survey', value: '2h ago', trend: 0, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'ROA'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Road Intelligence" breadcrumb="Roads" subtitle="AI-powered road condition monitoring and route optimization" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'analysis' && (<>
        <div className="grid grid-2">
          <ChartCard title="Road Intelligence — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [67, 79, 68, 55, 86, 94], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [41, 55, 47, 71, 53, 70], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [40, 20, 38, 25, 43], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={82.7} max={100} thresholds={[60, 80]} trend={1.2} />
          <GaugeChart title="Efficiency" value={91.1} max={100} thresholds={[55, 75]} trend={3.0} />
          <GaugeChart title="Compliance" value={91.2} max={100} thresholds={[80, 90]} trend={2.7} />
          <GaugeChart title="Utilization" value={78.2} max={100} thresholds={[50, 75]} trend={3.8} />
        </div>
      </>)}

      {tab === 'predictive' && (
        <TableCard title="Predictive" count={tableRows.length}>
      ...
      {tab === 'city' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [53, 70, 41, 41, 57, 80], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [59, 42, 62, 52, 63, 62], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [31, 26, 22, 39], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={80.8} max={100} thresholds={[70, 85]} trend={3.8} />
          <GaugeChart title="Coverage" value={78.7} max={100} thresholds={[60, 80]} trend={3.8} />
          <GaugeChart title="Accuracy" value={86.7} max={100} thresholds={[80, 90]} trend={1.4} />
          <GaugeChart title="Speed" value={77.3} max={100} thresholds={[50, 75]} trend={1.1} />
        </div>
      </>)}

      {tab === 'hazards' && (
        <TableCard title="Hazards" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [80, 85, 91, 73, 56, 74], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [31, 26, 22, 39], backgroundColor: '#10b981' },
            { label: 'Pending', data: [12, 7, 15, 4], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [6, 6, 7, 4], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={84.6} max={100} thresholds={[65, 80]} trend={3.1} />
          <GaugeChart title="Trend" value={69.6} max={100} thresholds={[55, 75]} trend={3.3} />
          <GaugeChart title="Target Hit" value={79.1} max={100} thresholds={[70, 85]} trend={3.9} />
          <GaugeChart title="Velocity" value={94.1} max={100} thresholds={[50, 70]} trend={-0.3} />
        </div>
      </>)}
