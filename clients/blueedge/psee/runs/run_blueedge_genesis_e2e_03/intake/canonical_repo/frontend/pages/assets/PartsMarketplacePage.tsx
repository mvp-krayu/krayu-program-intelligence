...
export default function PartsMarketplacePage() {
  ...
  const kpis = [
    { title: 'Catalog Size', value: 2450, trend: 120, sparkline: spark(2450) },
    { title: 'Active Orders', value: 34, trend: 5, sparkline: spark(34) },
    { title: 'Vendors', value: 18, trend: 2, sparkline: spark(18) },
    { title: 'Lead Time', value: '2.1 days', trend: -0.3, sparkline: spark(50) },
    { title: 'Cost Savings', value: '24%', trend: 3, sparkline: spark(50) },
    { title: 'Stock Alerts', value: 12, trend: -4, sparkline: spark(12) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'PAR'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Parts Marketplace" breadcrumb="Parts" subtitle="OEM parts procurement and inventory management" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'catalog' && (<>
        <div className="grid grid-2">
          <ChartCard title="Parts Marketplace — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [55, 75, 70, 58, 95, 69], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [50, 57, 34, 62, 68, 59], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [37, 35, 18, 26, 18], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={84.7} max={100} thresholds={[60, 80]} trend={2.7} />
          <GaugeChart title="Efficiency" value={71.7} max={100} thresholds={[55, 75]} trend={-0.4} />
          <GaugeChart title="Compliance" value={68.2} max={100} thresholds={[80, 90]} trend={4.1} />
          <GaugeChart title="Utilization" value={90.4} max={100} thresholds={[50, 75]} trend={3.6} />
        </div>
      </>)}

      {tab === 'orders' && (
        <TableCard title="Orders" count={tableRows.length}>
      ...
      {tab === 'vendors' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [44, 50, 48, 30, 48, 62], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [54, 43, 81, 38, 77, 40], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [18, 41, 43, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={76.0} max={100} thresholds={[70, 85]} trend={0.8} />
          <GaugeChart title="Coverage" value={87.9} max={100} thresholds={[60, 80]} trend={0.4} />
          <GaugeChart title="Accuracy" value={82.2} max={100} thresholds={[80, 90]} trend={3.0} />
          <GaugeChart title="Speed" value={89.2} max={100} thresholds={[50, 75]} trend={1.0} />
        </div>
      </>)}

      {tab === 'inventory' && (
        <TableCard title="Inventory" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [82, 91, 77, 68, 76, 63], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [18, 41, 43, 34], backgroundColor: '#10b981' },
            { label: 'Pending', data: [12, 11, 8, 5], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [3, 6, 6, 7], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={90.4} max={100} thresholds={[65, 80]} trend={1.2} />
          <GaugeChart title="Trend" value={94.2} max={100} thresholds={[55, 75]} trend={-0.5} />
          <GaugeChart title="Target Hit" value={89.6} max={100} thresholds={[70, 85]} trend={3.8} />
          <GaugeChart title="Velocity" value={83.4} max={100} thresholds={[50, 70]} trend={2.3} />
        </div>
      </>)}
