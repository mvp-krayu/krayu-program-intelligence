...
export default function WhiteLabelPage() {
  ...
  const kpis = [
    { title: 'Tenants', value: 12, trend: 2, sparkline: spark(12) },
    { title: 'Custom Themes', value: 8, trend: 1, sparkline: spark(8) },
    { title: 'Custom Domains', value: 5, trend: 1, sparkline: spark(5) },
    { title: 'Active Logos', value: 10, trend: 2, sparkline: spark(10) },
    { title: 'Branded Portals', value: 7, trend: 1, sparkline: spark(7) },
    { title: 'Setup Time', value: '2.1 hrs', trend: -0.3, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'WHI'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="White Label" breadcrumb="White Label" subtitle="Multi-tenant branding and customization management" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'themes' && (<>
        <div className="grid grid-2">
          <ChartCard title="White Label — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [75, 86, 79, 64, 80, 93], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [42, 66, 54, 52, 50, 63], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [19, 40, 39, 13, 41], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={77.8} max={100} thresholds={[60, 80]} trend={0.6} />
          <GaugeChart title="Efficiency" value={87.4} max={100} thresholds={[55, 75]} trend={1.9} />
          <GaugeChart title="Compliance" value={74.9} max={100} thresholds={[80, 90]} trend={3.1} />
          <GaugeChart title="Utilization" value={86.1} max={100} thresholds={[50, 75]} trend={4.4} />
        </div>
      </>)}

      {tab === 'tenants' && (
        <TableCard title="Tenants" count={tableRows.length}>
      ...
      {tab === 'domains' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [38, 51, 59, 46, 27, 72], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [44, 46, 75, 52, 79, 70], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [25, 15, 48, 18], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={73.1} max={100} thresholds={[70, 85]} trend={2.1} />
          <GaugeChart title="Coverage" value={72.5} max={100} thresholds={[60, 80]} trend={0.9} />
          <GaugeChart title="Accuracy" value={68.6} max={100} thresholds={[80, 90]} trend={0.0} />
          <GaugeChart title="Speed" value={70.8} max={100} thresholds={[50, 75]} trend={0.5} />
        </div>
      </>)}

      {tab === 'preview' && (
        <TableCard title="Preview" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [63, 81, 66, 77, 73, 75], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [25, 15, 48, 18], backgroundColor: '#10b981' },
            { label: 'Pending', data: [14, 10, 14, 13], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [6, 4, 7, 2], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={68.6} max={100} thresholds={[65, 80]} trend={0.6} />
          <GaugeChart title="Trend" value={81.1} max={100} thresholds={[55, 75]} trend={0.4} />
          <GaugeChart title="Target Hit" value={87.9} max={100} thresholds={[70, 85]} trend={-0.4} />
          <GaugeChart title="Velocity" value={70.6} max={100} thresholds={[50, 70]} trend={2.2} />
        </div>
      </>)}
