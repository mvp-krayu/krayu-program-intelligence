...
export default function AgenticAIPage() {
  ...
  const kpis = [
    { title: 'Active Agents', value: 12, trend: 2, sparkline: spark(12) },
    { title: 'Tasks (24h)', value: 456, trend: 38, sparkline: spark(456) },
    { title: 'Decisions', value: 1289, trend: 105, sparkline: spark(1289) },
    { title: 'Confidence', value: '94.2%', trend: 1.1, sparkline: spark(50) },
    { title: 'Cost Saved', value: '18,500 AED', trend: 2200, sparkline: spark(50) },
    { title: 'Overrides', value: 3, trend: -1, sparkline: spark(3) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'AI '.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Agentic AI" breadcrumb="AI Agents" subtitle="Autonomous AI agents for fleet optimization and operations" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'orchestrator' && (<>
        <div className="grid grid-2">
          <ChartCard title="Agentic AI — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [44, 75, 91, 95, 81, 64], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [34, 59, 69, 50, 61, 71], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [12, 38, 20, 23, 15], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={82.2} max={100} thresholds={[60, 80]} trend={4.0} />
          <GaugeChart title="Efficiency" value={94.8} max={100} thresholds={[55, 75]} trend={0.9} />
          <GaugeChart title="Compliance" value={73.9} max={100} thresholds={[80, 90]} trend={3.5} />
          <GaugeChart title="Utilization" value={94.3} max={100} thresholds={[50, 75]} trend={2.5} />
        </div>
      </>)}
      ...
      {tab === 'mesh' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [54, 56, 69, 34, 29, 73], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [66, 47, 81, 42, 58, 48], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [24, 15, 43, 47], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={84.7} max={100} thresholds={[70, 85]} trend={1.3} />
          <GaugeChart title="Coverage" value={73.2} max={100} thresholds={[60, 80]} trend={2.7} />
          <GaugeChart title="Accuracy" value={82.0} max={100} thresholds={[80, 90]} trend={2.6} />
          <GaugeChart title="Speed" value={85.6} max={100} thresholds={[50, 75]} trend={2.9} />
        </div>
      </>)}

      {tab === 'companion' && (
        <TableCard title="Companion" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [86, 62, 69, 73, 64, 76], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [24, 15, 43, 47], backgroundColor: '#10b981' },
            { label: 'Pending', data: [13, 5, 10, 8], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [5, 5, 4, 3], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={87.3} max={100} thresholds={[65, 80]} trend={3.1} />
          <GaugeChart title="Trend" value={66.5} max={100} thresholds={[55, 75]} trend={2.8} />
          <GaugeChart title="Target Hit" value={83.2} max={100} thresholds={[70, 85]} trend={3.7} />
          <GaugeChart title="Velocity" value={79.9} max={100} thresholds={[50, 70]} trend={2.6} />
        </div>
      </>)}
