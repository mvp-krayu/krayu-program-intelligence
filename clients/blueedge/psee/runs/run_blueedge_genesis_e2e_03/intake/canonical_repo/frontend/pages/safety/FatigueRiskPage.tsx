...
export default function FatigueRiskPage() {
  ...
  const kpis = [
    { title: 'Drivers Monitored', value: 156, trend: 5, sparkline: spark(156) },
    { title: 'High Risk', value: 4, trend: -1, sparkline: spark(4) },
    { title: 'Avg Risk Score', value: 22.5, trend: -3.2, sparkline: spark(22) },
    { title: 'Rest Violations', value: 7, trend: 2, sparkline: spark(7) },
    { title: 'Interventions', value: 34, trend: 8, sparkline: spark(34) },
    { title: 'Prevented', value: 89, trend: 12, sparkline: spark(89) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'FAT'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Fatigue Risk Management" breadcrumb="Fatigue" subtitle="Driver fatigue monitoring and risk assessment" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'monitor' && (<>
        <div className="grid grid-2">
          <ChartCard title="Fatigue Risk Management — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [92, 88, 57, 76, 62, 90], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [51, 42, 41, 55, 42, 56], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [41, 24, 18, 45, 28], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={83.3} max={100} thresholds={[60, 80]} trend={1.0} />
          <GaugeChart title="Efficiency" value={69.6} max={100} thresholds={[55, 75]} trend={-0.2} />
          <GaugeChart title="Compliance" value={87.9} max={100} thresholds={[80, 90]} trend={3.9} />
          <GaugeChart title="Utilization" value={81.2} max={100} thresholds={[50, 75]} trend={4.2} />
        </div>
      </>)}

      {tab === 'assessments' && (
        <TableCard title="Assessments" count={tableRows.length}>
      ...
      {tab === 'alerts' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [30, 28, 31, 60, 69, 48], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [38, 54, 85, 61, 84, 78], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [15, 40, 39, 34], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={88.4} max={100} thresholds={[70, 85]} trend={-0.1} />
          <GaugeChart title="Coverage" value={80.9} max={100} thresholds={[60, 80]} trend={1.9} />
          <GaugeChart title="Accuracy" value={65.0} max={100} thresholds={[80, 90]} trend={-0.2} />
          <GaugeChart title="Speed" value={74.7} max={100} thresholds={[50, 75]} trend={3.3} />
        </div>
      </>)}

      {tab === 'compliance' && (
        <TableCard title="HOS Compliance" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [74, 62, 89, 87, 66, 79], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [15, 40, 39, 34], backgroundColor: '#10b981' },
            { label: 'Pending', data: [7, 13, 5, 10], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [4, 8, 7, 3], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={65.6} max={100} thresholds={[65, 80]} trend={3.3} />
          <GaugeChart title="Trend" value={92.9} max={100} thresholds={[55, 75]} trend={0.1} />
          <GaugeChart title="Target Hit" value={91.4} max={100} thresholds={[70, 85]} trend={1.9} />
          <GaugeChart title="Velocity" value={89.9} max={100} thresholds={[50, 70]} trend={2.2} />
        </div>
      </>)}
