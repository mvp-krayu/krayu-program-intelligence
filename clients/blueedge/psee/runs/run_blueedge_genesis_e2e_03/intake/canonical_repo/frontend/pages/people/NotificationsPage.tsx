...
export default function NotificationsPage() {
  ...
  const kpis = [
    { title: 'Total Today', value: 342, trend: 15, sparkline: spark(342) },
    { title: 'Unread', value: 18, trend: -5, sparkline: spark(18) },
    { title: 'Critical', value: 5, trend: 1, sparkline: spark(5) },
    { title: 'Auto-Resolved', value: 127, trend: 22, sparkline: spark(127) },
    { title: 'Channels', value: 4, trend: 0, sparkline: spark(4) },
    { title: 'Avg Response', value: '8 min', trend: -2, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'NOT'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Notification Center" breadcrumb="Notifications" subtitle="Real-time alerts and notification management" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'inbox' && (<>
        <div className="grid grid-2">
          <ChartCard title="Notification Center — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [50, 93, 55, 46, 64, 80], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [44, 57, 53, 47, 56, 41], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [35, 10, 42, 29, 35], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={79.2} max={100} thresholds={[60, 80]} trend={0.5} />
          <GaugeChart title="Efficiency" value={92.0} max={100} thresholds={[55, 75]} trend={-0.4} />
          <GaugeChart title="Compliance" value={88.9} max={100} thresholds={[80, 90]} trend={0.7} />
          <GaugeChart title="Utilization" value={70.1} max={100} thresholds={[50, 75]} trend={1.9} />
        </div>
      </>)}

      {tab === 'rules' && (
        <TableCard title="Rules" count={tableRows.length}>
      ...
      {tab === 'channels' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [66, 62, 45, 29, 62, 43], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [51, 77, 71, 50, 50, 55], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [29, 25, 19, 30], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={67.5} max={100} thresholds={[70, 85]} trend={3.7} />
          <GaugeChart title="Coverage" value={80.5} max={100} thresholds={[60, 80]} trend={-0.1} />
          <GaugeChart title="Accuracy" value={84.0} max={100} thresholds={[80, 90]} trend={1.6} />
          <GaugeChart title="Speed" value={75.1} max={100} thresholds={[50, 75]} trend={2.6} />
        </div>
      </>)}

      {tab === 'templates' && (
        <TableCard title="Templates" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [71, 76, 79, 72, 71, 63], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [29, 25, 19, 30], backgroundColor: '#10b981' },
            { label: 'Pending', data: [17, 13, 17, 12], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [3, 5, 1, 3], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={89.6} max={100} thresholds={[65, 80]} trend={0.5} />
          <GaugeChart title="Trend" value={87.5} max={100} thresholds={[55, 75]} trend={3.0} />
          <GaugeChart title="Target Hit" value={85.2} max={100} thresholds={[70, 85]} trend={2.0} />
          <GaugeChart title="Velocity" value={71.7} max={100} thresholds={[50, 70]} trend={0.7} />
        </div>
      </>)}
