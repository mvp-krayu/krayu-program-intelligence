...
export default function ExecutivePage() {
  ...
  const kpis = [
    ...
    { title: 'Margin', value: '33.3%', trend: 1.2, sparkline: spark(50) },
    { title: 'Utilization', value: '87.5%', trend: 2.5, sparkline: spark(50) },
    { title: 'Safety', value: 94.2, trend: 1.8, sparkline: spark(94) },
    { title: 'NPS', value: 72, trend: 5, sparkline: spark(72) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'EXE'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Executive Dashboard" breadcrumb="Executive" subtitle="C-suite real-time fleet intelligence and business KPIs" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'overview' && (<>
        <div className="grid grid-2">
          <ChartCard title="Executive Dashboard — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [92, 42, 60, 64, 92, 87], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [34, 61, 55, 74, 46, 48], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [17, 14, 40, 26, 33], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={80.8} max={100} thresholds={[60, 80]} trend={1.8} />
          <GaugeChart title="Efficiency" value={87.4} max={100} thresholds={[55, 75]} trend={4.3} />
          <GaugeChart title="Compliance" value={79.3} max={100} thresholds={[80, 90]} trend={-0.2} />
          <GaugeChart title="Utilization" value={88.3} max={100} thresholds={[50, 75]} trend={1.9} />
        </div>
      </>)}

      {tab === 'financial' && (
        <TableCard title="Financial" count={tableRows.length}>
      ...
      {tab === 'operations' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [60, 58, 26, 68, 38, 32], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [63, 38, 73, 45, 46, 78], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [26, 20, 47, 15], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={80.4} max={100} thresholds={[70, 85]} trend={1.5} />
          <GaugeChart title="Coverage" value={68.3} max={100} thresholds={[60, 80]} trend={2.9} />
          <GaugeChart title="Accuracy" value={80.1} max={100} thresholds={[80, 90]} trend={2.0} />
          <GaugeChart title="Speed" value={93.4} max={100} thresholds={[50, 75]} trend={4.0} />
        </div>
      </>)}

      {tab === 'safety' && (
        <TableCard title="Safety" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'benchmarks' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [84, 64, 67, 61, 57, 82], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [26, 20, 47, 15], backgroundColor: '#10b981' },
            { label: 'Pending', data: [16, 5, 5, 7], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [2, 6, 1, 1], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={66.3} max={100} thresholds={[65, 80]} trend={-0.1} />
          <GaugeChart title="Trend" value={88.5} max={100} thresholds={[55, 75]} trend={-0.1} />
          <GaugeChart title="Target Hit" value={91.0} max={100} thresholds={[70, 85]} trend={2.5} />
          <GaugeChart title="Velocity" value={80.6} max={100} thresholds={[50, 70]} trend={-0.2} />
        </div>
      </>)}
