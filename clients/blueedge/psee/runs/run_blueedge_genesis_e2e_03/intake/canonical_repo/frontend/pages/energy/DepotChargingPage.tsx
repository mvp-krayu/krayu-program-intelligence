...
export default function DepotChargingPage() {
  ...
  const kpis = [
    { title: 'Chargers', value: 64, trend: 4, sparkline: spark(64) },
    { title: 'Available', value: 28, trend: 2, sparkline: spark(28) },
    { title: 'Charging', value: 36, trend: 2, sparkline: spark(36) },
    { title: 'Load (kW)', value: 2840, trend: 120, sparkline: spark(2840) },
    { title: 'Queue', value: 8, trend: -2, sparkline: spark(8) },
    { title: 'Utilization', value: '87.5%', trend: 3.2, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'DEP'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Depot Charging" breadcrumb="Depot" subtitle="Fleet depot charging infrastructure and scheduling" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'live' && (<>
        <div className="grid grid-2">
          <ChartCard title="Depot Charging — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [87, 94, 47, 60, 71, 58], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [51, 42, 41, 34, 43, 47], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [32, 19, 40, 16, 21], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={80.1} max={100} thresholds={[60, 80]} trend={0.5} />
          <GaugeChart title="Efficiency" value={87.9} max={100} thresholds={[55, 75]} trend={2.0} />
          <GaugeChart title="Compliance" value={73.4} max={100} thresholds={[80, 90]} trend={2.0} />
          <GaugeChart title="Utilization" value={94.7} max={100} thresholds={[50, 75]} trend={-0.3} />
        </div>
      </>)}

      {tab === 'schedule' && (
        <TableCard title="Schedule" count={tableRows.length}>
      ...
      {tab === 'vehicles' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [57, 42, 67, 52, 53, 52], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [50, 36, 82, 60, 83, 46], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [27, 17, 32, 46], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={85.4} max={100} thresholds={[70, 85]} trend={0.2} />
          <GaugeChart title="Coverage" value={68.6} max={100} thresholds={[60, 80]} trend={1.2} />
          <GaugeChart title="Accuracy" value={94.3} max={100} thresholds={[80, 90]} trend={1.9} />
          <GaugeChart title="Speed" value={76.8} max={100} thresholds={[50, 75]} trend={1.8} />
        </div>
      </>)}

      {tab === 'power' && (
        <TableCard title="Power Grid" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [69, 70, 76, 62, 75, 58], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [27, 17, 32, 46], backgroundColor: '#10b981' },
            { label: 'Pending', data: [13, 10, 11, 16], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [4, 7, 6, 6], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={88.8} max={100} thresholds={[65, 80]} trend={2.5} />
          <GaugeChart title="Trend" value={75.2} max={100} thresholds={[55, 75]} trend={2.1} />
          <GaugeChart title="Target Hit" value={93.2} max={100} thresholds={[70, 85]} trend={1.1} />
          <GaugeChart title="Velocity" value={87.6} max={100} thresholds={[50, 70]} trend={2.6} />
        </div>
      </>)}
