...
export default function AnomalyDetectionPage() {
  ...
  const kpis = [
    { title: 'Active', value: 34, trend: 5, sparkline: spark(34) },
    { title: 'Resolved (7d)', value: 127, trend: 18, sparkline: spark(127) },
    { title: 'Confidence', value: '91.5%', trend: 2.3, sparkline: spark(50) },
    { title: 'Vehicles', value: 18, trend: -2, sparkline: spark(18) },
    { title: 'False Positive', value: '3.2%', trend: -0.8, sparkline: spark(50) },
    { title: 'Models', value: 6, trend: 1, sparkline: spark(6) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'ANO'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Anomaly Detection" breadcrumb="Anomalies" subtitle="AI-powered anomaly detection across fleet telemetry" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'active' && (<>
        <div className="grid grid-2">
          <ChartCard title="Anomaly Detection — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [90, 76, 57, 64, 72, 80], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [34, 43, 64, 38, 36, 54], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [44, 29, 42, 39, 19], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={71.6} max={100} thresholds={[60, 80]} trend={0.2} />
          <GaugeChart title="Efficiency" value={66.9} max={100} thresholds={[55, 75]} trend={1.8} />
          <GaugeChart title="Compliance" value={89.7} max={100} thresholds={[80, 90]} trend={2.9} />
          <GaugeChart title="Utilization" value={92.3} max={100} thresholds={[50, 75]} trend={3.2} />
        </div>
      </>)}

      {tab === 'history' && (
        <TableCard title="History" count={tableRows.length}>
      ...
      {tab === 'models' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [70, 52, 69, 66, 44, 31], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [83, 42, 83, 78, 71, 84], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [49, 43, 28, 43], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={74.1} max={100} thresholds={[70, 85]} trend={4.2} />
          <GaugeChart title="Coverage" value={77.2} max={100} thresholds={[60, 80]} trend={1.6} />
          <GaugeChart title="Accuracy" value={69.2} max={100} thresholds={[80, 90]} trend={3.2} />
          <GaugeChart title="Speed" value={93.4} max={100} thresholds={[50, 75]} trend={0.3} />
        </div>
      </>)}

      {tab === 'patterns' && (
        <TableCard title="Patterns" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [59, 64, 56, 88, 76, 89], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [49, 43, 28, 43], backgroundColor: '#10b981' },
            { label: 'Pending', data: [3, 11, 10, 13], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [6, 5, 7, 8], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={74.1} max={100} thresholds={[65, 80]} trend={1.6} />
          <GaugeChart title="Trend" value={79.8} max={100} thresholds={[55, 75]} trend={-0.0} />
          <GaugeChart title="Target Hit" value={67.9} max={100} thresholds={[70, 85]} trend={1.9} />
          <GaugeChart title="Velocity" value={91.6} max={100} thresholds={[50, 70]} trend={1.5} />
        </div>
      </>)}
