...
export default function LStopTransitPage() {
  ...
  const kpis = [
    { title: 'Active eBuses', value: 184, trend: 12, sparkline: spark(184) },
    { title: 'Routes', value: 24, trend: 2, sparkline: spark(24) },
    { title: 'Passengers', value: 12480, trend: 850, sparkline: spark(12480) },
    { title: 'Avg SOC', value: '72%', trend: 3, sparkline: spark(50) },
    { title: 'BESS', value: '4.8 MWh', trend: 0.2, sparkline: spark(50) },
    { title: 'CO₂ Saved', value: '42.5t', trend: 5.2, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'L-S'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="L-STOP Electric Transit" breadcrumb="L-STOP" subtitle="Electric bus transit management with BESS integration" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'command' && (<>
        <div className="grid grid-2">
          <ChartCard title="L-STOP Electric Transit — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [69, 68, 94, 70, 61, 66], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [59, 74, 41, 31, 65, 46], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [36, 32, 37, 36, 22], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={87.3} max={100} thresholds={[60, 80]} trend={3.6} />
          <GaugeChart title="Efficiency" value={91.8} max={100} thresholds={[55, 75]} trend={2.2} />
          <GaugeChart title="Compliance" value={76.6} max={100} thresholds={[80, 90]} trend={1.1} />
          <GaugeChart title="Utilization" value={94.2} max={100} thresholds={[50, 75]} trend={-0.1} />
        </div>
      </>)}

      {tab === 'battery' && (
        <TableCard title="Battery & BESS" count={tableRows.length}>
      ...
      {tab === 'charging' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [27, 55, 70, 35, 68, 51], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [70, 67, 76, 38, 74, 58], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [25, 17, 22, 16], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={79.9} max={100} thresholds={[70, 85]} trend={2.8} />
          <GaugeChart title="Coverage" value={79.9} max={100} thresholds={[60, 80]} trend={1.0} />
          <GaugeChart title="Accuracy" value={92.7} max={100} thresholds={[80, 90]} trend={2.5} />
          <GaugeChart title="Speed" value={80.6} max={100} thresholds={[50, 75]} trend={1.6} />
        </div>
      </>)}

      {tab === 'routes' && (
        <TableCard title="Routes" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [84, 87, 67, 60, 69, 88], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [25, 17, 22, 16], backgroundColor: '#10b981' },
            { label: 'Pending', data: [17, 11, 18, 11], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [3, 6, 2, 3], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={89.0} max={100} thresholds={[65, 80]} trend={2.9} />
          <GaugeChart title="Trend" value={86.8} max={100} thresholds={[55, 75]} trend={1.3} />
          <GaugeChart title="Target Hit" value={67.4} max={100} thresholds={[70, 85]} trend={-0.3} />
          <GaugeChart title="Velocity" value={83.1} max={100} thresholds={[50, 70]} trend={3.9} />
        </div>
      </>)}
