...
export default function UsersPage() {
  ...
  const kpis = [
    { title: 'Total Users', value: 24, trend: 3, sparkline: spark(24) },
    { title: 'Active Now', value: 12, trend: 2, sparkline: spark(12) },
    { title: 'Admins', value: 3, trend: 0, sparkline: spark(3) },
    { title: 'Drivers', value: 45, trend: 5, sparkline: spark(45) },
    { title: 'Avg Session', value: '42 min', trend: -3, sparkline: spark(50) },
    { title: 'Login Rate', value: '99.2%', trend: 0.1, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'USE'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="User Management" breadcrumb="Users" subtitle="Manage platform users, roles, and permissions" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'list' && (<>
        <div className="grid grid-2">
          <ChartCard title="User Management — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [63, 93, 95, 71, 80, 49], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [43, 74, 56, 54, 64, 33], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [30, 28, 40, 16, 44], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={91.4} max={100} thresholds={[60, 80]} trend={2.8} />
          <GaugeChart title="Efficiency" value={91.4} max={100} thresholds={[55, 75]} trend={4.2} />
          <GaugeChart title="Compliance" value={76.1} max={100} thresholds={[80, 90]} trend={0.2} />
          <GaugeChart title="Utilization" value={69.7} max={100} thresholds={[50, 75]} trend={0.1} />
        </div>
      </>)}

      {tab === 'roles' && (
        <TableCard title="Roles" count={tableRows.length}>
      ...
      {tab === 'activity' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [29, 35, 58, 62, 38, 32], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [80, 47, 65, 66, 56, 64], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [33, 48, 22, 40], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={90.0} max={100} thresholds={[70, 85]} trend={0.0} />
          <GaugeChart title="Coverage" value={86.1} max={100} thresholds={[60, 80]} trend={2.3} />
          <GaugeChart title="Accuracy" value={83.4} max={100} thresholds={[80, 90]} trend={0.9} />
          <GaugeChart title="Speed" value={94.6} max={100} thresholds={[50, 75]} trend={2.5} />
        </div>
      </>)}

      {tab === 'sessions' && (
        <TableCard title="Sessions" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === '__never__' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [92, 92, 58, 63, 65, 90], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [33, 48, 22, 40], backgroundColor: '#10b981' },
            { label: 'Pending', data: [7, 9, 13, 7], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [3, 6, 2, 4], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={84.6} max={100} thresholds={[65, 80]} trend={3.1} />
          <GaugeChart title="Trend" value={65.2} max={100} thresholds={[55, 75]} trend={0.5} />
          <GaugeChart title="Target Hit" value={89.5} max={100} thresholds={[70, 85]} trend={2.7} />
          <GaugeChart title="Velocity" value={74.0} max={100} thresholds={[50, 70]} trend={0.8} />
        </div>
      </>)}
