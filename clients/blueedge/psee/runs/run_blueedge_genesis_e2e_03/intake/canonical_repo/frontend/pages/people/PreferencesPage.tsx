...
export default function PreferencesPage() {
  ...
  const kpis = [
    { title: 'Settings Groups', value: 5, trend: 0, sparkline: spark(5) },
    { title: 'Custom Configs', value: 3, trend: 1, sparkline: spark(3) },
    { title: 'Theme', value: 'Dark Mode', trend: 0, sparkline: spark(50) },
    { title: 'Language', value: 'English', trend: 0, sparkline: spark(50) },
    { title: 'Timezone', value: 'Asia/Dubai', trend: 0, sparkline: spark(50) },
    { title: 'Currency', value: 'AED', trend: 0, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'PRE'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="User Preferences" breadcrumb="Preferences" subtitle="Customize your dashboard experience" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'general' && (<>
        <div className="grid grid-2">
          <ChartCard title="User Preferences — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [55, 88, 63, 52, 70, 80], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [39, 44, 75, 59, 50, 53], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [14, 18, 22, 31, 18], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={78.5} max={100} thresholds={[60, 80]} trend={4.1} />
          <GaugeChart title="Efficiency" value={77.7} max={100} thresholds={[55, 75]} trend={3.7} />
          <GaugeChart title="Compliance" value={73.4} max={100} thresholds={[80, 90]} trend={0.3} />
          <GaugeChart title="Utilization" value={72.5} max={100} thresholds={[50, 75]} trend={1.9} />
        </div>
      </>)}

      {tab === 'notifications' && (
        <TableCard title="Notifications" count={tableRows.length}>
      ...
      {tab === 'display' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [37, 29, 60, 38, 75, 72], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [39, 47, 68, 46, 42, 82], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [35, 32, 42, 43], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={92.7} max={100} thresholds={[70, 85]} trend={0.6} />
          <GaugeChart title="Coverage" value={78.3} max={100} thresholds={[60, 80]} trend={1.5} />
          <GaugeChart title="Accuracy" value={90.8} max={100} thresholds={[80, 90]} trend={-0.2} />
          <GaugeChart title="Speed" value={81.5} max={100} thresholds={[50, 75]} trend={1.4} />
        </div>
      </>)}

      {tab === 'data' && (
        <TableCard title="Data & Privacy" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'shortcuts' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [59, 70, 68, 87, 64, 62], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [35, 32, 42, 43], backgroundColor: '#10b981' },
            { label: 'Pending', data: [6, 4, 9, 9], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [4, 6, 6, 8], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={66.5} max={100} thresholds={[65, 80]} trend={4.4} />
          <GaugeChart title="Trend" value={95.0} max={100} thresholds={[55, 75]} trend={0.8} />
          <GaugeChart title="Target Hit" value={90.1} max={100} thresholds={[70, 85]} trend={3.4} />
          <GaugeChart title="Velocity" value={94.1} max={100} thresholds={[50, 70]} trend={1.8} />
        </div>
      </>)}
