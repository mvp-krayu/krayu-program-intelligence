...
export default function ElectrificationPage() {
  ...
  const kpis = [
    { title: 'Plans', value: 5, trend: 1, sparkline: spark(5) },
    { title: 'EV Target', value: 120, trend: 15, sparkline: spark(120) },
    { title: 'Current EVs', value: 45, trend: 8, sparkline: spark(45) },
    { title: 'Savings', value: '2.4M AED', trend: 0.3, sparkline: spark(50) },
    { title: 'CO₂ Cut', value: '340t', trend: 42, sparkline: spark(50) },
    { title: 'Payback', value: '32 mo', trend: -2, sparkline: spark(50) },
  ];

  const tableRows = Array.from({ length: 8 }, (_, i) => ({
    id: `${'ELE'.slice(0,3)}-${String(i+1).padStart(3,'0')}`,
  ...
  return (
    <div>
      <PageHeader title="Electrification Planning" breadcrumb="Electrification" subtitle="Fleet transition roadmap from ICE to electric vehicles" />
      <div className="grid grid-3" style={{ marginBottom: 20 }}>{{}kpis.map((k, i) => <TrendCard key={i} {...k} />){}}</div>
      <TabBar tabs={TABS} active={tab} onChange={setTab} />

      {tab === 'roadmap' && (<>
        <div className="grid grid-2">
          <ChartCard title="Electrification Planning — Trend" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Current', data: [84, 55, 47, 77, 61, 94], borderColor: '#22d3ee', fill: true, backgroundColor: 'rgba(34,211,238,0.1)' },
            { label: 'Previous', data: [67, 73, 66, 43, 43, 62], borderColor: '#64748b', borderDash: [5,5] },
          ] }} />
          <ChartCard title="Distribution" type="doughnut" data={{ labels: ['Tanker','Bus','Taxi','EV','Other'], datasets: [{ data: [22, 25, 19, 27, 17], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6','#ef4444'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Performance" value={86.7} max={100} thresholds={[60, 80]} trend={4.1} />
          <GaugeChart title="Efficiency" value={93.1} max={100} thresholds={[55, 75]} trend={3.9} />
          <GaugeChart title="Compliance" value={92.4} max={100} thresholds={[80, 90]} trend={-0.5} />
          <GaugeChart title="Utilization" value={70.3} max={100} thresholds={[50, 75]} trend={2.3} />
        </div>
      </>)}

      {tab === 'plans' && (
        <TableCard title="Plans" count={tableRows.length}>
      ...
      {tab === 'financial' && (<>
        <div className="grid grid-2">
          <ChartCard title="Monthly Comparison" type="bar" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Actual', data: [55, 76, 63, 33, 59, 57], backgroundColor: '#22d3ee' },
            { label: 'Target', data: [47, 68, 62, 67, 38, 56], backgroundColor: '#334155' },
          ] }} />
          <ChartCard title="Category Split" type="doughnut" data={{ labels: ['Category A','Category B','Category C','Category D'], datasets: [{ data: [40, 19, 42, 15], backgroundColor: ['#22d3ee','#f59e0b','#10b981','#8b5cf6'] }] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Quality" value={91.5} max={100} thresholds={[70, 85]} trend={0.0} />
          <GaugeChart title="Coverage" value={70.3} max={100} thresholds={[60, 80]} trend={4.4} />
          <GaugeChart title="Accuracy" value={92.6} max={100} thresholds={[80, 90]} trend={0.9} />
          <GaugeChart title="Speed" value={94.9} max={100} thresholds={[50, 75]} trend={4.4} />
        </div>
      </>)}

      {tab === 'environmental' && (
        <TableCard title="Environmental" count={6}>
          <CrudDataTable columns={[
            { label: 'ID', render: (r: any) => r.id },
      ...
      {tab === 'analytics' && (<>
        <div className="grid grid-2">
          <ChartCard title="Trend Analysis" type="line" data={{ labels: ['Sep','Oct','Nov','Dec','Jan','Feb'], datasets: [
            { label: 'Performance', data: [67, 79, 89, 58, 76, 75], borderColor: '#22d3ee' },
          ...
          <ChartCard title="Weekly Distribution" type="bar" data={{ labels: ['Week 1','Week 2','Week 3','Week 4'], datasets: [
            { label: 'Completed', data: [40, 19, 42, 15], backgroundColor: '#10b981' },
            { label: 'Pending', data: [11, 17, 9, 17], backgroundColor: '#f59e0b' },
            { label: 'Overdue', data: [7, 4, 2, 8], backgroundColor: '#ef4444' },
          ] }} />
        </div>
        <div className="grid grid-4" style={{ marginTop: 16 }}>
          <GaugeChart title="Overall" value={76.9} max={100} thresholds={[65, 80]} trend={2.2} />
          <GaugeChart title="Trend" value={79.9} max={100} thresholds={[55, 75]} trend={2.0} />
          <GaugeChart title="Target Hit" value={93.1} max={100} thresholds={[70, 85]} trend={4.2} />
          <GaugeChart title="Velocity" value={93.9} max={100} thresholds={[50, 70]} trend={3.8} />
        </div>
      </>)}
