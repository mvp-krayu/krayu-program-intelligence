#!/usr/bin/env python3
"""
PSEE Generic Intake Replay
Stream: PSEE.RECONCILE.1.WP-11

Replays PiOS intake deterministically for any client using the built package.

Usage:
    python3 scripts/psee/run_intake_replay_generic.py --client <client_id> --run-id <run_id>

Reads:   clients/<client_id>/psee/runs/<run_id>/package/
...
# ── ARGUMENT PARSING ──────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="PSEE Generic Intake Replay")
...
INTAKE_DIR = os.path.join(REPO_ROOT, "clients", CLIENT_ID, "psee", "runs", RUN_ID, "intake")

os.makedirs(INTAKE_DIR, exist_ok=True)

print("=== PSEE Generic Intake Replay ===")
    ...
    for artifact in MANDATORY_ARTIFACTS:
        ...
        if os.path.isfile(path):
            ...
            if art_run_id != manifest_run_id:
                log(f"  FAIL  {artifact}: run_id={art_run_id} != {manifest_run_id}")
                errors.append(f"RUN_ID_INCONSISTENCY: {artifact} run_id={art_run_id}")
                rejected = True
            else:
...
if not rejected:
    for artifact in MANDATORY_ARTIFACTS:
        ...
        if os.path.isfile(path):
            ...
            errors.append(f"PACKAGE_INCOMPLETE: missing {artifact}")
            rejected = True
...
# ── STEP 6: DETERMINE SCOPE AND UNCERTAINTY ──────────────────────────────────
log()
...
if intake_mode == "AUTHORITATIVE_INTAKE":
    ...
    log(f"  PASS_PARTIAL → verified scope authoritative")
    if unverified_scope_lines:
        log(f"  unverified: {unverified_scope_lines}")
else:
    consumed_scope = "none"
...
log("  WRITTEN  intake_result.json")
