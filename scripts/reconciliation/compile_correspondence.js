...
/**
 * Compile reconciliation correspondence artifact for any client/run.
 * PI.SQO.RECONCILIATION-CORRESPONDENCE-COMPILER.01
 *
 ...
 *
 * Usage:
 *   node scripts/reconciliation/compile_correspondence.js --client <client_id> --run <run_id>
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--client' && argv[i + 1]) { args.client = argv[++i]; }
    else if (argv[i] === '--run' && argv[i + 1]) { args.run = argv[++i]; }
  }
  return args;
}
...
if (!args.client || !args.run) {
  console.error('Usage: node compile_correspondence.js --client <client_id> --run <run_id>');
  process.exit(1);
}
