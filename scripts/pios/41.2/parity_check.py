...
Compares generated PIE vault against canonical docs/pios/41.2/pie_vault/.

Comparison strategy:
  1. File set equality (same relative paths in both)
  2. Byte-level content equality per file
  3. [[wiki-link]] structure equality for domain, capability, component files

Reports EXACT_MATCH (all checks pass, exit 0) or DIFF (any mismatch, exit 1).

Usage:
  python3 parity_check.py --generated /tmp/pios_41_2_verify --canonical docs/pios/41.2/pie_vault
  python3 parity_check.py --generated /tmp/pios_41_2_verify --canonical docs/pios/41.2/pie_vault --verbose
"""
...
def phase1_file_set_equality(gen: dict, can: dict, report: ParityReport):
    """Check that both vaults contain exactly the same set of relative paths."""
    ...
    if only_generated:
        for k in sorted(only_generated):
            report.add_diff(f"file_set: extra in generated", k)
    if only_canonical:
        for k in sorted(only_canonical):
            report.add_diff(f"file_set: missing from generated", k)
    if not only_generated and not only_canonical:
        report.add_match(f"file_set: {len(common)} files match exactly")

    return common


def phase2_content_equality(common: set, gen: dict, can: dict, report: ParityReport,
                             verbose: bool = False):
    ...
    if diff_count == 0:
        report.add_match(f"content: all {match_count} files byte-identical")
    return diff_count


def phase3_wiki_link_structure(common: set, gen: dict, can: dict, report: ParityReport):
    """
    Verify [[wiki-link]] structure equality for markdown files.
    This is a secondary check — content equality already catches these,
    but link structure is surfaced explicitly for diagnosis.
    """
    ...
    for rel in sorted(md_files):
        ...
        if gen_links == can_links:
            # Only log mismatches to keep output clean
            pass
        else:
            link_diffs += 1
    ...
    if link_diffs == 0:
        report.add_match(f"wiki_links: all {len(md_files)} markdown files have identical [[link]] structure")
    return link_diffs
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare generated PIE vault against canonical docs/pios/41.2/pie_vault/."
    )
    parser.add_argument(
        "--generated",
        default="/tmp/pios_41_2_output",
        help="Generated vault directory (default: /tmp/pios_41_2_output)",
    )
    parser.add_argument(
        "--canonical",
        default="docs/pios/41.2/pie_vault",
        help="Canonical vault directory (default: docs/pios/41.2/pie_vault)",
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
    ...
    if report.is_exact_match():
        ...
        print(f"Generated vault is structurally and byte-identical to canonical.")
        print(f"  Files compared  : {len(common)}")
        print(f"  Differences     : 0")
        print("Safe to promote to docs/pios/41.2/pie_vault/ if desired.")
        sys.exit(0)
    else:
